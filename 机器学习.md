...
1. **提高前一轮中被分类错误的样本的权重，降低被正确分类的样本的权重**
2. **各个弱分类器的组合是采取加权多数表决的方式——加大错误率较低的弱分类器的权重，减小错误率较高的弱分类器的权重**
...
每一轮训练一个新的弱学习器，用来**拟合**当前模型的**残差**，也就是**预测值和真实值之间的差距**。每次加入的模型都是为了**修正前一轮模型的错误**。

因为每一步朝着损失函数的方向的方向前进，因此被称为梯度提升
...
激活函数定义了该节点在给定的输入或输入集合下的输出，给神经网络引入非线性能力，使其能学习复杂的特征和映射关系。

激活函数分为：

+ 饱和：Sigmoid函数，Tanh函数。在极大/小的输入时，梯度几乎为0(消失，导致参数无法得到有效的更新)
+ 非饱和：ReLU。不会包含，但是负区间恒为0。
...
卷积的目的是提取图像的特征，根据不同的卷积核可以得到不同的特征提取图
